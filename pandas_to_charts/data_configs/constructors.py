...
from pandas_to_charts.data_configs.basic_types import Plotly2DSeries, Plotly3DSeries, PlotlyPieSeries, \
    NamedPoint2DSeries, NamedPoint2D, NamedPoint3DSeries, NamedPoint3D
...
def get_highcharts_2d_constructor(df: pd.DataFrame, x: str, y: str) -> NamedPoint2DSeries:
    return NamedPoint2DSeries(
        data=[NamedPoint2D(name=x_, y=y_) for x_, y_ in
              zip(df[x].values.tolist(), df[y].values.tolist())]
    )
