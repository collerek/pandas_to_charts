...
from pandas_to_charts.data_configs.constructors import (
    ChartTypes,
    get_chart_constructor,
    Libraries,
)
...
def make_chart_data(
    ...
    for group_name in group_names:
        if len(grouper) == 1 and grouper[0] == one_group:
            ...
            names.append(
                ", ".join(["{}".format(b) for a, b in zip(grouper, group_name)])
            )
    ...
    if chart_type == ChartTypes.heatmap:
        ...
        trace.type = ChartTypes._value2member_map_[chart_type.value].name  # type: ignore
        ...
        for i, group_name in enumerate(data_series.group_names):
            for serie in y:
                ...
                trace = generate_trace(
                    final_group, df, x, chart_type, library, serie=serie
                )
...
def fill_missing_values(
    df: pd.DataFrame, data_series: DataSerie, group_name: list, x: str
) -> pd.DataFrame:
    group = data_series.grouped_data.get_group(
        group_name if len(group_name) > 1 else group_name[0]
    )
